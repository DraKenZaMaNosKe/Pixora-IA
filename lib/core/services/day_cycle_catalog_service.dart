...
class DayCycleCatalogService {
  ...
  List<DayCycleTheme> get themes => _themes;
  ...
  Future<List<DayCycleTheme>> fetchCatalog({bool forceRefresh = false}) async {
    if (_themes.isNotEmpty && _isCacheValid && !forceRefresh) {
      return _themes;
    }
  ...
  Future<void> _saveToCache(String json) async {
    try {
      ...
      debugPrint('[Pixora] Day cycle cache save failed: $e');
    }
  }
  ...
  Future<List<DayCycleTheme>?> _loadFromCache() async {
    try {
      ...
      debugPrint('[Pixora] Day cycle cache load failed: $e');
    }
    return null;
  }
}
