"""Publica Ares tras QA aprobado en Huawei."""
...
def validate(spec: dict, hidden: bool) -> None:
    ...
    if {layer.get("key") for layer in layers} != {"background", "forge_aura", "ares"}:
    ...
    for key in ("forge_aura", "ares"):
        layer = next(layer for layer in layers if layer.get("key") == key)
        if not close(layer.get("scale", -1), 1.0) or not close(layer.get("parallax_factor", -1), .17):
            raise RuntimeError("Paralaje sincronizado cambio")
        if not close(layer.get("bob_amplitude_px", -1), .9) or not close(layer.get("bob_period_sec", -1), 7.2):
...
def main() -> None:
    ...
    if len(matches) != 1 or matches[0].get("published") is not False:
        raise RuntimeError("Ares no parte oculto o esta duplicado")
