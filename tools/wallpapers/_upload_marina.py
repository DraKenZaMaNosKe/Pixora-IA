"""Upload Marina · Poder de Agua (Sailor Mercury) — canonical v2 formula.

Same pattern as Broly/Sailor Moon, no animated sprite (Marina's water
spiral is already baked into the subject PNG).

  - fondo: cover-fit 1080x2340, parallax=0.0 (static)
  - marina (subject): bake_subject_complete v2, parallax=0.85

Assets in C:/Users/lalo/Desktop/wallPapers_repo/nuevos/marina_sailor_moon/:
  - marina_soloFondo.png            (1080x1920 background)
  - marina_sailormoon_solopersonage.png (1080x1920 transparent subject)
  - marina_wallpaper_estatico.png   (composited flat)
"""
import json
import re
import shutil
...
TARGET = (1080, 2340)
SCENE_ID = "marina_sailor_moon"
STATIC_ID = "marina_sailor_moon_static"
SUBJECT_KEY = "marina"

SRC = Path(r"C:/Users/lalo/Desktop/wallPapers_repo/nuevos/marina_sailor_moon")
WORK = Path(r"D:/Orbix/Pixora-IA/tools/wallpapers/_tmp_marina")
if WORK.exists():
    shutil.rmtree(WORK)
...
# ============================================================
print("=" * 60)
print("Upload Marina · Poder de Agua — v2 canonical formula")
print("=" * 60)
...
fondo_img = cover_fit_rgb(Image.open(SRC / "marina_soloFondo.png"))
...
subject_baked, runtime_scale = bake_subject_complete(
    Image.open(SRC / "marina_sailormoon_solopersonage.png"))
...
flat_img = cover_fit_rgb(Image.open(SRC / "marina_wallpaper_estatico.png"))
...
TITLE = {"es": "Marina · Poder de Agua", "en": "Sailor Mercury · Water Power"}
TAGS = ["sailor_moon", "sailor_mercury", "marina", "anime", "magical_girl",
        "agua", "water", "3d", "parallax"]
...
cur.execute(
    ...
    """,
    (
        SCENE_ID, "Marina · Poder de Agua",
        "Sailor Mercury (Marina) invocando su poder de agua bajo la luna — parallax 3D",
        TAGS, f"{SCENE_ID}.webp", f"{SCENE_ID}_preview.webp",
        len(flat_body), len(prev_body), "#3BB6E8", sort, "Pixora Studio",
        TARGET[0], TARGET[1],
    ),
)
...
STATIC_TAGS = ["sailor_moon", "sailor_mercury", "marina", "anime", "magical_girl"]
cur.execute("SELECT COALESCE(MAX(sort_order), 0) + 1 FROM wallpapers;")
sort = cur.fetchone()[0]
cur.execute(
    """
    ...
    """,
    (
        STATIC_ID, "Marina · Poder de Agua · Estatico",
        "Sailor Mercury (Marina) — imagen estatica sin parallax",
...
print()
print("=" * 60)
print("DONE — Marina · Poder de Agua uploaded (canonical v2)")
