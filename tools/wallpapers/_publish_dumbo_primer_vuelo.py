"""Publica Dumbo: el primer vuelo entre sueños tras aprobación en Huawei."""
...
def head_size(url: str) -> int:
    ...
    with urllib.request.urlopen(req, timeout=30) as response:
        if response.status != 200:
            raise RuntimeError(f"HEAD falló: {response.status} {url}")
...
def main() -> None:
    ...
    if sprite.get("frame_skip") != 10 or params.get("high_res") is not False:
        raise RuntimeError("Configuración de rendimiento inesperada")
    if abs(float(params.get("x", 0)) - 0.52) > 0.000001:
        raise RuntimeError("La posición X aprobada cambió")
    if abs(float(params.get("y", 0)) - 0.47) > 0.000001:
        raise RuntimeError("La posición Y aprobada cambió")
    if abs(float(params.get("scale", 0)) - 0.00315) > 0.000001:
    ...
    if int(manifest_entry.get("frames", 0)) != 12:
        raise RuntimeError("El manifest no confirma los 12 cuadros")
    ...
    if sprite_zip_size != int(manifest_entry.get("size", 0)):
        raise RuntimeError("El tamaño remoto del ZIP no coincide con el manifest")
    ...
    receipt = {
        ...
        "qa_capture": "qa/HUAWEI_QA_V2.png",
