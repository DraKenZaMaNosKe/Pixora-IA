"""Publica Artemisa tras QA aprobado en Huawei."""
...
def validate(spec: dict, hidden: bool) -> None:
    ...
    if {layer.get("key") for layer in layers} != {"background", "moon_aura", "artemis_and_doe"}:
    ...
    for key in ("moon_aura", "artemis_and_doe"):
        layer = next(layer for layer in layers if layer.get("key") == key)
        if not close(layer.get("scale", -1), 1.0) or not close(layer.get("parallax_factor", -1), .15):
            raise RuntimeError("Paralaje sincronizado cambio")
        if not close(layer.get("bob_amplitude_px", -1), .7) or not close(layer.get("bob_period_sec", -1), 7.8):
...
def main() -> None:
    ...
    if len(matches) != 1 or matches[0].get("published") is not False:
        raise RuntimeError("Artemisa no parte oculta o esta duplicada")
