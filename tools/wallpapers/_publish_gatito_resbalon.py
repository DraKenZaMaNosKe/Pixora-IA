"""Publica Gatito contra la gravedad tras QA final en Huawei."""
...
def main() -> None:
    ...
    if len(matches) != 1:
        raise RuntimeError(f"Entradas de catalogo: {len(matches)}")
    ...
    if sprite.get("manifest_key") != "gatito_resbalon_pared_gato":
        raise RuntimeError("Manifest inesperado")
    if sprite.get("frame_skip") != 24 or params.get("high_res") is not False:
        raise RuntimeError("Configuracion de rendimiento inesperada")
    if abs(float(params.get("scale", 0)) - 0.00520) > 0.000001:
        raise RuntimeError("Escala final del gatito inesperada")
    background = layers[0]
    if background.get("key") != "background" or abs(float(background.get("scale", 0)) - 1.24) > 0.001:
        raise RuntimeError("Cobertura final inesperada")

    manifest = get_json("wallpaper-sprites", "manifest.json")
    manifest_entry = manifest.get("gatito_resbalon_pared_gato", {})
    if int(manifest_entry.get("frames", 0)) != 12:
        raise RuntimeError("El manifest no confirma 12 frames")
    ...
    sprite_zip_size = head_size(f"{PUBLIC}/wallpaper-sprites/gatito_resbalon_pared_gato.zip")
    ...
    if not remote_spec.get("published") or len(remote_matches) != 1 or not remote_matches[0].get("published"):
        raise RuntimeError("La verificacion remota de publicacion fallo")
    ...
    receipt = {
        ...
        "background_revision": background.get("revision"),
        "asset_sizes": {
            "flat": image_size, "preview": preview_size,
            "background": background_size, "sprite_zip": sprite_zip_size,
        },
        "qa_device": "HUAWEI VNS-L53",
        "qa_capture": "qa/QA_HUAWEI_FINAL_PREPROD.png",
