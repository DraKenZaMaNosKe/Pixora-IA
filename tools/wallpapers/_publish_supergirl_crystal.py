"""Promueve Supergirl Crystal Hope de QA privado a produccion publica."""
...
def main() -> None:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"Se esperaba una entrada de catalogo; encontradas: {len(matches)}"
        )

    spec = get_json(SCENES_BUCKET, f"{SID}.json")
    if len(spec.get("image_layers", [])) != 6 or len(spec.get("cycles", [])) != 1:
        raise RuntimeError("Spec inesperado: se cancela la promocion")
