"""Publica Hermes tras QA aprobado en Huawei."""
...
def validate(spec: dict, hidden: bool) -> None:
    ...
    if {layer.get("key") for layer in layers} != {"background", "solar_aura", "apollo_full"}:
    ...
    for key in ("solar_aura", "apollo_full"):
        layer = next(layer for layer in layers if layer.get("key") == key)
        if not close(layer.get("scale", -1), 1.0) or not close(layer.get("parallax_factor", -1), .16):
            raise RuntimeError("Paralaje sincronizado cambio")
        if not close(layer.get("bob_amplitude_px", -1), .7) or not close(layer.get("bob_period_sec", -1), 7.6):
...
def main() -> None:
    ...
    approved_spec = json.loads((ROOT / "SCENE_SPEC_QA.json").read_text(encoding="utf-8"))
    if spec != approved_spec:
        raise RuntimeError("El spec remoto cambio respecto al QA aprobado")
    ...
    if len(matches) != 1 or matches[0].get("published") is not False:
        raise RuntimeError("Apolo no parte oculto o esta duplicado")
    ...
    try:
        ...
        expected_remote = json.loads(json.dumps(approved_spec))
        expected_remote["published"] = True
        if remote != expected_remote:
            raise RuntimeError("La publicacion altero campos fuera de published")
        ...
        try:
            if not send_catalog_invalidate("wallpapers"):
                print("ADVERTENCIA: fallo FCM compensatorio durante rollback", file=sys.stderr)
        except Exception:
            print("ADVERTENCIA: excepcion en FCM compensatorio durante rollback", file=sys.stderr)
        cur.close(); conn.close()
        raise
    cur.close(); conn.close()
