...
download the high-quality preview MP3 to tools/aura/raw/.

Reads the API token from KEYS_LOCAL.md (line 'API Key (client secret): <key>').
"""
import json
import re
import sys
...
def main() -> None:
    ...
    for t in manifest["nature"]:
        ...
        hit = search(token, q)
        if not hit:
            print("   NO RESULT — adjust query in manifest")
        ...
        try:
            ...
            log.append({"id": t["id"], "status": "download_error", "query": q, "error": str(e)})
            continue
